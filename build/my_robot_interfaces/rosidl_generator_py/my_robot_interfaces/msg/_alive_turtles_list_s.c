...
bool my_robot_interfaces__msg__alive_turtles_list__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("my_robot_interfaces.msg._alive_turtles_list.AliveTurtlesList", full_classname_dest, 60) == 0);
  }
  my_robot_interfaces__msg__AliveTurtlesList * ros_message = _ros_message;
  {  // x
  ...
  {  // theta
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      float * dest = ros_message->theta.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
...
{
  /* NOTE(esteve): Call constructor of AliveTurtlesList */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("my_robot_interfaces.msg._alive_turtles_list");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "AliveTurtlesList");
  ...
  {  // theta
    ...
    if (ros_message->theta.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      float * src = &(ros_message->theta.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->theta.size * sizeof(float));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
